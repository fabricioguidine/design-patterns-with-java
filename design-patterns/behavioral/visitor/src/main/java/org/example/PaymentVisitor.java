...
public class PaymentVisitor implements Visitor {
    ...
    public String displayPixPayment(PixPayment pixPayment) {
        return "PixPayment{" + "pixKey=" + pixPayment.getPixKey() + ", value=" + pixPayment.getValue() + '}';
    }

    @Override
    public String displayCreditCardPayment(CreditCardPayment creditCardPayment) {
        return "CreditCardPayment{" + "cardNumber="
                + creditCardPayment.getCardNumber() + ", expiryDate='"
                + creditCardPayment.getExpiryDate() + '\'' + ", securityCode='"
                + creditCardPayment.getSecurityCode() + '\'' + ", value="
                + creditCardPayment.getValue() + '}';
    }

    @Override
    public String displayInvoicePayment(InvoicePayment invoicePayment) {
        return "InvoicePayment{" + "barcode="
                + invoicePayment.getBarcode() + ", value="
                + invoicePayment.getValue() + '}';
    }
}
